...
function Products() {
  ...
  const [listCartIncluded, setListCartIncluded] = useState([]);
  ...
  const hasDifferentListPriceUsed = () => {
    for (const item of listCartIncluded) {
  ...
  const [nextFocusField, setNextFocusField] = useState(null);
  ...
  useEffect(() => {
    if (
      ...
      setNextFocusField(null);
    }
  }, [listCart, nextFocusField]);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <input
            ...
            ref={el => (inputRefs.current.quantidade[row.index] = el)}
            onBlur={e => handleBlur(e, row, 'quantidade')}
      ...
      },
      {
        ...
        Cell: ({ row }) => (
          <input
            type="text"
            defaultValue={row.original.ITEM_VALOR_UNITARIO_ESCOLHIDO}
            ref={el => (inputRefs.current.valorUnitario[row.index] = el)}
            onBlur={e => handleBlur(e, row, 'valor')}
            onClick={e => e.target.select()}
            onWheel={e => e.target.blur()}
            disabled={row.original.QUANTIDADE > 0 ? false : true}
            style={{
              width: '100%',
              height: '30px',
              fontSize: '16px',
    ...
    [listCart, isEditing, selectedPriceList],
  );
  ...
  const handleBlur = useCallback(
    debounce((e, row, type) => {
      ...
      const previousValue =
        row.original[type === 'quantidade' ? 'QUANTIDADE' : 'ITEM_VALOR_UNITARIO_ESCOLHIDO'];

      if (newValue !== previousValue) {
        // Chama a função de envio para o backend
        sendCartDataToBackend(row, newValue, type);
      } else {
        if (isEditing === false) {
          handleFocus(row.index, type);
        }
    ...
    [isEditing, sendCartDataToBackend, handleFocus],
  );
