...
function Products() {
  ...
  const focusNextInput = (rowIndex, columnIndex) => {
    const nextKey = `${rowIndex}-${columnIndex + 1}`;
    const nextInput = inputRefs.current[nextKey];

    // Caso o próximo input na linha não exista, vá para a primeira coluna da próxima linha
    if (!nextInput) {
      const nextRowKey = `${rowIndex + 1}-0`;
      if (inputRefs.current[nextRowKey]) {
        inputRefs.current[nextRowKey].focus(); // Foco no próximo input da próxima linha
      }
    } else {
      nextInput.focus(); // Foco no próximo input da mesma linha
    }
  };
  ...
  const sendCartDataToBackend = (row, newQuantity, newValue, type, id) => {
  ...
  const validarValorUnitario = (newValue, row, listRules) => {
    if (
      Number(listRules[0].LISTA_PRECOS_VALOR_PERMITIDO_INFERIOR_AO_UNITARIO) >=
        0 &&
  ...
  const handleFocus = (index, type) => {
    ...
    // Se a coluna for 0, move para a próxima coluna da mesma linha
    if (col === 0) {
      nextIndex = `${row}-${col + 1}`;
    }
    // Se a coluna for 1, move para a primeira coluna da próxima linha
    else if (col === 1) {
      nextIndex = `${row + 1}-0`;
    }
...
const handleFocus = (index, type, key) => {
  console.log(index)
  console.log(type)
  if (type === 'quantidade') {
    const isValorUnitarioEnabled = !inputRefs.current[index]?.disabled;
    if (isValorUnitarioEnabled) {
      inputRefs.current[`${index + 1}-0`]?.focus();
    } else {
      setNextFocusField({ field: 'quantidade', key: key + 1 });
    }
  } else if (type === 'valor') {
    setNextFocusField({ field: 'quantidade', key: key + 1 });
  }
...
*/
  const nextFocusRef = useRef(null); // Referência para o próximo input a ser focado

  useEffect(() => {
    if (nextFocusRef.current) {
      const nextInput = inputRefs.current[nextFocusRef.current];
      if (nextInput) {
        nextInput.focus();
        nextFocusRef.current = null; // Limpar após focar
      }
    }
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        minWidth: 300,
        sortType: 'basic',
        Cell: ({ value }) => <div style={{ fontSize: '18px' ,minWidth:'300px'}}>{value}</div>,
      },
      {
        Header: 'Variante',
        accessor: 'ITEM_GRADE',
...
*/
          const handleBlurValorUnitario = useCallback(
            e => {
              ...
              if (
                parseFloat(newValue) !=
                parseFloat(inputValorUnitarioRef.current)
              ) {
                console.log('aquiii')
                const validationResult = validarValorUnitario(
                  newValue,
                  row,
                  listRules,
                );
                console.log( 'aquii22222')
                ...
                >
                  ...
                  <Grid item sm={1} lg={1} md={1}>
                    <Button
                      ...
                      style={{ width: '100%' }}
                    >
                    {showInput ? <ArrowForwardIcon /> : <MenuIcon />}
        </Button>
                  </Grid>
