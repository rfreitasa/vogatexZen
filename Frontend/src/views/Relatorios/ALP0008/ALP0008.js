...
function ALP0008() {
  ...
  const loadCompany = async () => {
    try {
      ...
      const data = response.data
        ? response.data.data
            .filter(item => item.EMPRESA_NOME != 'PERSONAL SOFTWARE')
            .map(item => {
              return {
  ...
  const loadbusinessGroup = async (inputValue, callback) => {
    try {
      const response = await axios.get(
        `${API.businessGroup}?parametro=${inputValue}`,
        {
          headers: {
            'x-access-token': token,
          },
  ...
  const loadcity = async (inputValue, callback) => {
    try {
      const response = await axios.get(
        `${API.cidadeErp}?parametro=${inputValue}`,
        {
          headers: {
            'x-access-token': token,
          },
  ...
  const debouncedLoadOptionbusinessGroup = debounce(
    loadOptionsbusinessGroup,
    1000,
    {
      leading: true,
    },
  ...
  const handleSearch = async data => {
    try {
      ...
      const campos = Object.keys(opcoesEscolhidas).filter(
        key => opcoesEscolhidas[key],
      );
      const labels = campos.map(
        campo => opcoesCampos.find(opcao => opcao.campo === campo).label,
      );
      const tamanhos = campos.map(campo =>
        parseInt(opcoesCampos.find(opcao => opcao.campo === campo).size),
      );
      ...
      if (vendedorEscolhido > 0) {
        data.salespersonIds = vendedorEscolhido;
      }

      if (data.supervisorId > 0) {
        if (autoListSupervisor[0].VENDEDORES) {
          data.salespersonIds = autoListSupervisor[0].VENDEDORES;
        }
        //  console.log(autoListSupervisor)
      }
      if (city.length > 0) {
        const cidades_selecionadas = verificaEAtualizaArray(city);
        data.cityIds = cidades_selecionadas;
      }
      if (state.length > 0) {
        ...
        data.stateIds = state_escolhidos;
      }

      data.group1 = data.group1;
      const axios_search = `${API.relatorios}/?relatorio=ALP0008&email=${email}`;
      const parametros = {
      ...
      <>
        <Grid container spacing={1}>
          <Grid item xs={12} sm={12} lg={2}>
            <div className="input">
              <label>Dias inativos</label>
              <input
                type="number"
                name="inactivityDaysStart"
                defaultValue={90}
                ref={register}
              />
            </div>
          ...
          <Grid key="Agrupamento" item xs={12} sm={12} lg={4}>
            <div className="input">
              ...
              <select name="group1" ref={register}>
                <option value="salesperson_nome">Vendedor</option>
              </select>
            </div>
          </Grid>
        </Grid>
...
export default connect()(ALP0008);
