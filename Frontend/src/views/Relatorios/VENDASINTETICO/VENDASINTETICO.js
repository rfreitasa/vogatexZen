...
function VENDAANALITICO() {
  ...
  const handleSearch = async data => {
    try {
      ...
      var where = `&agrupamento='${data.agrupamento}'&filtro_data='${data.filtro_data}'&dt_inicial='${data.dt_inicial}'&dt_final='${data.dt_final}'&cliente=${clienteEscolhido}&vendedor=${vendedorEscolhido}&gerente=${gerenteEscolhido}&produto=${produtoEscolhido}&status=${statusEscolhido}`;
      //console.log(`${API.relatorios}/?relatorio=VENDASINTETICO&email=${email}${where}`);
      axios_search = `${API.relatorios}/?relatorio=VENDASINTETICO&email=${email}${where}`;
  ...
  const loadOptionsClientes = (inputValue, callback) =>
    inputValue.length > 3 ? loadClients(inputValue, callback) : '';
  ...
  const debouncedLoadOptionSellers = debounce(loadOptionsSellers, 2000, {
    leading: true,
  });
