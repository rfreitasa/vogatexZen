...
function NOTASFISCAIS() {
  ...
  const loadSellers = async (inputValue, callback) => {
    try {
      const response = await axios.get(
        `${API.vendedores}?email=${email}&parametro=${inputValue}`,
        {
          headers: {
  ...
  const loadstates = async (inputValue, callback) => {
    try {
      const response = await axios.get(
        `${API.estados}?parametro=${inputValue}`,
        {
          headers: {
            'x-access-token': token,
          },
  ...
  const montarOpcoes = () => {
    ...
    opcoesCampos.forEach(campo => {
      ...
      escolhasIniciais[campo.campo] =
        escolhaSalva || campo.default == 's' ? true : false;
    });
      ...
      if (perfil != 'vendedor') {
        ...
        function encontrarVendedores(supervisorId, arrayDeObjetos) {
          return arrayDeObjetos
            .filter(
              objeto => objeto.USUARIO_CONTA_SUPERVISOR_ID === supervisorId,
            )
        ...
        setAutoListSupervisor(
          data.filter(function(obj) {
            ...
            if (perfil == 'supervisor') {
              return (
                obj.USUARIO_PERFIL === 'supervisor' &&
                obj.USUARIO_EMAIL.trim().toUpperCase() ==
  ...
  const handleSearch = async data => {
    ...
    try {
      ...
      if (data.supervisorId > 0) {
        const supervisor = autoListSupervisor.find(
          sup => Number(sup.USUARIO_ID) === Number(data.supervisorId),
        );

        if (supervisor) {
          if (supervisor.VENDEDORES) {
            data.salesperson = supervisor.VENDEDORES;
          }
        }
      }
      ...
      if (statusPedidos.length > 0) {
        const status_selecionados = verificaEAtualizaArray(statusPedidos);

        data.statusList = status_selecionados;
      }
      if (state.length > 0) {
        const state_escolhidos = JSON.stringify(
          state.map(item => {
            return item.value;
          }),
        );
      ...
      if (idProduto.length > 0) {
        ...
        data.product = produtomestre_selecionados;
      }
      if (idProdutoGrade.length > 0) {
        const produtograde_selecionados = verificaEAtualizaArray(
          idProdutoGrade,
        );
        data.productPacking = produtograde_selecionados;
      }

      var axios_search = `${API.relatorios}/?relatorio=NOTASFISCAIS&email=${email}`;
      const parametros = {
        data, // Mantém os dados existentes
      };
      if (perfil == 'vendedor' && vendedorEscolhido == '') {
        axios_search = '';
      }
      try {
    ...
    return (
      <>
        <Grid container spacing={1}>
          ...
          <Grid key="vendedor" item xs={12} sm={12} lg={5}>
            <div className="input">
              <label>Vendedor</label>

              <Async
                //onChange={e => (itemRequest.prazo_pagamentor = e.target.value)}
                loadOptions={debouncedLoadOptionSellers}
                cacheOptions
                styles={{
                  menuPortal: base => ({
                    ...base,
          ...
          <Grid key="empresas" item xs={12} sm={12} lg={2}>
            <div className="input">
          ...
          <Grid key="supervisor" item xs={6} sm={12} lg={3}>
            <div className="input">
              ...
              <Autocomplete
                renderInput={props => <input {...props} autoComplete={false} />}
                items={autoListSupervisor}
                shouldItemRender={(item, value) =>
                  item.USUARIO_NOME.toLowerCase().indexOf(value.toLowerCase()) >
                  -1
                }
                getItemValue={item => {
          ...
          <Grid key="Agrupamento" item xs={12} sm={12} lg={2}>
            <div className="input">
              <label>Agrupamento</label>

              <select name="group1" ref={register}>
                <option value="invoice_date">Emissão</option>
                <option value="person_name">Cliente</option>
                <option value="salesperson_name">Vendedor</option>

                <option value="company_code">Empresa</option>
                <option value="fiscalProfileOperation_code">
                  Operação Fiscal
                </option>
                <option value="product_description">Produto Mestre</option>
              </select>
            </div>
          </Grid>
          <Grid key="qtd" item xs={12} sm={12} lg={2}>
            <div className="input">
          ...
          <Grid key="ordenacao" item xs={12} sm={12} lg={2}>
            <div className="input">
      ...
      <Pesquisa>
        <div>
          <ExpansionPanel expanded={PanelOpen}>
            ...
            >
              <Typography component={'span'}>Painel de pesquisa</Typography>
            </ExpansionPanelSummary>
